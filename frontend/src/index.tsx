...
const DistanceTraveledStats = ({ value }: { value: number }) => (
  <>
    <span id="distance-traveled" style={`--value:${value.toString()};`}></span> km
  </>
)

const TripsCompletedStats = ({ count, percent }: { count: number, percent: number }) => {
  const change = count * (percent / 100)
  return <>
    <div class="stat-value">{count.toFixed(0)}</div>
    <div class="stat-desc">↗︎ {change.toFixed(2)} ({percent}%)</div>
  </>
}

const AverageTipStats = ({ average, percent }: { average: number, percent: number }) => {
  const change = average * (percent / 100) 
  return <>
    <div class="stat-value">{average.toFixed(2)} $</div>
    <div class="stat-desc">↗︎ {change.toFixed(3)} $ ({percent}%)</div>
  </>
}
...
const app = new Elysia()
  ...
  .get('/stats_stream', () => {
    ...
    const interval_1 = setInterval(() => {
      ...
      stream.send(DistanceTraveledStats({ value: i }))
    }, 3000)

    const interval_2 = setInterval(() => {
      const count = getRandomIntInclusive(1000, 7000)
      const percent = getRandomIntInclusive(1, 40)
      stream.event = "trips-completed"
      stream.send(TripsCompletedStats({count, percent }))
    }, 3500)

    const interval_3 = setInterval(() => {
      const average = getRandomIntInclusive(1, 600) / 100
      const percent = getRandomIntInclusive(1, 40)
      stream.event = "average-tip"
      stream.send(AverageTipStats({ average, percent }))
    }, 2050)
        ...
        <div class="flex flex-col justify-center content-center">
          ...
          <div class="stats shadow-lg">
            <div class="stat">
              ...
              <div hx-ext="sse" sse-connect="/stats_stream" sse-swap="distance-traveled" class="stat-value countdown">
            ...
            <div class="stat">
              ...
              <div hx-ext="sse" sse-connect="/stats_stream" sse-swap="trips-completed">
                <div class="stat-value loading loading-bars loading-md"></div>
            ...
            <div class="stat">
              ...
              <div hx-ext="sse" sse-connect="/stats_stream" sse-swap="average-tip">
                <div class="stat-value loading loading-bars loading-md"></div>
                <div class="stat-desc">↗︎ 0 (0%)</div>
              </div>
            </div>
